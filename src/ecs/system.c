...
void UpdateCollider(void* userdata, Entity* entity, struct QuadTree* tree)
{
	...
	QuadTreeClear(tree);
	if (entity->components.collider != NULL && entity->components.transform)
	{
		entity->components.collider->shape.box.x = entity->components.collider->offset.x + entity->components.transform->position.x;
		entity->components.collider->shape.box.y = entity->components.collider->offset.y + entity->components.transform->position.y;
	}
}
...
void UpdateInput(void* userdata, Entity* entity)
{
	...
	{
		if (entity->components.input->crank && entity->components.transform) 
		{
			double angle_rad = api->system->getCrankAngle() * (3.14159265358979323846f / 180.0f);
			entity->components.transform->position.x = (float)(0.9f * cos(angle_rad));
			entity->components.transform->position.y = (float)(0.7f * sin(angle_rad));
		} 
		// ...
	}
}
